              ...
              <v-list-item-content>
                ...
                <v-list dense flat>
                  <v-list-item v-for="(tab, k) in link.tabs" :key="k" class="font-medium tw-block">
                    ...
                    <div v-else-if="tab.id === 'resource'" class="text-base text-link tw-p-2"
                      @click="routerToArticle(tab.query)">
                      {{ superscriptTM(tab.title) }}
                    </div>
                    <nuxt-link v-else :to="'/' + data.category" class="text-base text-link tw-p-2 tw-block"
            ...
            <v-list-item-content class="tw-pt-0">
              ...
              <v-list dense flat>
                ...
                <v-list-item v-for="(tab, k) in link.tabs" :key="k" class="font-medium">
                  ...
                  <nuxt-link v-else :to="'/' + data.category" class="text-base text-link tw-p-2">{{ tab.title
                    }}</nuxt-link>
                </v-list-item>
              </v-list>
            </v-list-item-content>
